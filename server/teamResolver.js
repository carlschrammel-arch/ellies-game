 ...
 */
export function resolveTeam(userText) {
    var normalized = userText.toLowerCase().trim()
        .replace(/players?/gi, '')
        .replace(/baseball/gi, '')
        .replace(/team/gi, '')
        .trim();
    // Check aliases first
    var aliasMatch = teamAliases[normalized];
    if (aliasMatch) {
        return mlbTeams.find(function (t) { return t.slug === aliasMatch; }) || null;
    }
    // Direct slug match
    var slugMatch = mlbTeams.find(function (t) { return t.slug === normalized; });
    if (slugMatch)
        return slugMatch;
    // Name contains match
    var containsMatch = mlbTeams.find(function (t) {
        return normalized.includes(t.name.toLowerCase()) ||
    ...
    var _loop_1 = function (word) {
        if (word.length < 3)
            return "continue";
        var wordMatch = mlbTeams.find(function (t) {
            return t.name.toLowerCase() === word ||
                t.slug === word ||
                t.city.toLowerCase() === word;
        });
        if (wordMatch)
            return { value: wordMatch };
    };
 ...
 */
export function detectSportsTeamQuery(input) {
    var lower = input.toLowerCase();
    // Check for team-related keywords
    var teamKeywords = ['players', 'team', 'roster', 'baseball', 'mlb'];
    var hasTeamKeyword = teamKeywords.some(function (kw) { return lower.includes(kw); });
    // Try to resolve team
    var team = resolveTeam(input);
    return {
